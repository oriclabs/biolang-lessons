...
- Priority-2 source ports that passed the strict one-to-one fidelity gate:
  ...
  - `lessons/biostatistics/bdsr-nhanes-group-tests`: exact Welch, ANOVA, and
    Tukey results with a source-matched BioLang smoking/BMI boxplot.
  ...
  - `lessons/biostatistics/bdsr-essential-statistics-extended`: five connected
    notebooks restoring sections 8.3.2-8.6, including the exact Wilcoxon,
    nested regression, categorical-model, power, and model-tidying workflows;
    all four source analytical figures are generated by BioLang.
  - `lessons/biostatistics/bdsr-lung-survival`: exact `survival::lung`
    observations, source Kaplan-Meier calculations, and all three selected
    survival figures generated by BioLang.
...
- Five earlier, broader *Biological Data Science with R* companions are retained under
  ...
  that book, plus the connected 8.3.2-8.6 restoration listed above.
