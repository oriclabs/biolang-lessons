...
- Priority-2 source ports that passed the strict one-to-one fidelity gate:
  - `lessons/biostatistics/bdsr-nhanes-eda`: the bounded NHANES descriptive
    and EDA sequence with source-hosted data and six BioLang-generated,
    source-matched figures.
  - `lessons/biostatistics/bdsr-nhanes-group-tests`: all three source Welch
    questions, exact ANOVA and Tukey results, restored Understanding panels,
    and a source-matched BioLang smoking/BMI boxplot.
  - `lessons/biostatistics/bdsr-nhanes-regression`: exact height-weight OLS
    and both source-matched BioLang regression figures.
  - `lessons/biostatistics/bdsr-essential-statistics-extended`: five connected
    notebooks restoring sections 8.3.2-8.7, including the exact Wilcoxon,
    nested regression, categorical-model, power, and model-tidying workflows;
    the Understanding panels and closing design guidance are present, and all
    four analytical figures are generated by BioLang.
  - `lessons/biostatistics/bdsr-lung-survival`: the complete Chapter 9 sequence
    through 9.3.2, including all Understanding panels, Exercises 1-13, exact
    lung/colon/RTCGA analyses, and every analytical figure generated by BioLang.
  - `lessons/transcriptomics/bdsr-rnaseq`: the complete Chapter 12 sequence,
    all fourteen exercises, exact real airway inputs, historical DESeq2 1.44.0
    oracle parity, and every analytical plot generated by BioLang.
...
Registry discovery currently withholds the ten older lesson entries identified
in the runtime-figure audit because they still embed copied analytical figures.
Their source directories remain available for the one-to-one BioLang plotting
work; they are not presented to learners as completed runtime ports.
...
- Verified historical *Biological Data Science with R* Chapter 8 and Chapter 9
  packages are retained under `archive/superseded-bdsr`; their active coverage
  is provided by the NHANES lessons through section 8.7 and the complete
  `bdsr-lung-survival` lesson. The predictive and ggtree drafts remain under
  `archive/p2-non-fidelity-bdsr` because substituted
  algorithms or made-up/random tree data do not meet the current
  one-to-one/no-synthetic-data requirement.
- `lessons/transcriptomics/bdsr-rnaseq` restores BDSR Chapter 12 after a
  file-by-file re-audit: exact real airway inputs, all fourteen exercises,
  historical DESeq2 1.44.0 oracle parity, and every analytical plot generated
  by BioLang.
